 ...
 */
directives.directive("varnamala", function($rootScope){
	return {
		restrict: "A",
		link: function(scope, element){
			var s = Snap("#svg");
			var delta = 6.8; //tick angles
			...
			function drawTabs(){
				...
				function clickHandler() {
					...
					selectedTab = this;
					centerLabel = s.text(this.getBBox().x,this.getBBox().y, String.fromCharCode(2309+selectedIdx)).addClass('centreLabel')
						.animate({transform:'t' + (x-this.getBBox().x - 20) + ',' + (y-this.getBBox().y + 15) + 's1.3' }, 1000, mina.easeinout);

					createjs.Sound.play("" + selectedIdx);
				}
				...
				function drawVarnamala(){
					var startAngle = -delta+4;
					var endAngle = startAngle + delta;

					for(var i = 0; i< 360/delta; i++){
						var path = describeArc(x,y,rT, startAngle , endAngle);
						var label = s.text(0, y, String.fromCharCode(2309+i)) //x=0 for curved text
